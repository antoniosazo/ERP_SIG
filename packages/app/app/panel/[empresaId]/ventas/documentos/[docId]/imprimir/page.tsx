...
export default async function ImprimirDocumentoVentaPage({
  params,
}: {
  ...
  const cliente = terceros.find((t) => t.id === d.terceroId);
  ...
  return (
    <div id="doc-print" className="mx-auto max-w-3xl text-neutral-900">
      ...
      <header className="flex items-start justify-between border-b-2 border-neutral-800 pb-3">
        <div>
          <h1 className="text-lg font-semibold">{d.clase}</h1>
      ...
      <dl className="my-4 grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-3">
        {filaDato("Cliente", d.nombreCliente ?? cliente?.razonSocial ?? "—")}
        {filaDato("RUT", cliente?.rut ? formatearRut(cliente.rut) : "—")}
        ...
        {d.numAtCard ? filaDato("N° ref. del cliente", d.numAtCard) : null}
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-y border-neutral-400 text-left text-[11px] tracking-wide text-neutral-500 uppercase">
            ...
            <th className="py-1.5 pr-2">Cuenta de ingreso</th>
        ...
        <tbody>
          {lineas.map((l) => (
            <tr key={l.id} className="border-b border-neutral-200 align-top">
              ...
              <td className="py-1.5 pr-2">{cuentaNombre.get(l.cuentaIngresoId) ?? "—"}</td>
              <td className="py-1.5 pr-2">
              ...
              <td className="py-1.5 text-right tabular-nums">
                {l.impuestoId ? `${impuestoNombre.get(l.impuestoId) ?? ""} ${clp(l.montoImpuesto)}` : "—"}
              </td>
            </tr>
          ))}
        </tbody>
